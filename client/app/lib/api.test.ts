...
const { interceptorHandlers, mockAxiosInstance, mockAxiosCreate, mockAxiosPost } = vi.hoisted(() => {
  const handlers = {
    requestSuccess: null as ((config: InternalAxiosRequestConfig) => InternalAxiosRequestConfig) | null,
  ...
  const instance = Object.assign(instanceFn, {
    interceptors: {
      request: {
        use: vi.fn((success: (config: InternalAxiosRequestConfig) => InternalAxiosRequestConfig, error: (error: unknown) => Promise<never>) => {
      ...
      response: {
        use: vi.fn((success: (response: unknown) => unknown, error: (error: AxiosError) => Promise<unknown>) => {
...
describe('API client', () => {
  ...
  describe('request interceptor', () => {
    ...
    it('rejects errors in request interceptor', async () => {
      ...
      await expect(interceptorHandlers.requestError!(error)).rejects.toThrow('Request error');
    });
  });
  ...
  describe('response interceptor - error handling', () => {
    ...
    it('updates Authorization header on retry request', async () => {
      ...
      const originalConfig = { headers: {}, url: '/protected-resource' } as InternalAxiosRequestConfig & { _retry?: boolean };
      const error = {
        response: { status: 401 },
    ...
    it('marks request as retry to prevent infinite loop', async () => {
      ...
      const originalConfig = { headers: {} } as InternalAxiosRequestConfig & { _retry?: boolean };
      const error = {
        response: { status: 401 },
        config: originalConfig,
        isAxiosError: true,
