...
import { hasAuthToken, getRedirectTo, requireAuth, requireGuest } from './auth.server';
...
describe('auth.server', () => {
  describe('hasAuthToken', () => {
    ...
    it('should return true when refreshToken cookie is present', () => {
      const request = createMockRequest('http://localhost/', 'refreshToken=true');
  ...
  describe('getRedirectTo', () => {
    ...
    it('should return redirectTo param when valid', () => {
      const request = createMockRequest('http://localhost/login?redirectTo=/products');
      expect(getRedirectTo(request)).toBe('/products');
    });

    it('should return default when redirectTo is absolute URL (security)', () => {
      const request = createMockRequest('http://localhost/login?redirectTo=https://evil.com');
      expect(getRedirectTo(request)).toBe('/dashboard');
    });

    it('should return default when redirectTo starts with // (security)', () => {
      const request = createMockRequest('http://localhost/login?redirectTo=//evil.com');
      expect(getRedirectTo(request)).toBe('/dashboard');
    });
  ...
  describe('requireAuth', () => {
    ...
    it('should not throw when authenticated', () => {
      const request = createMockRequest('http://localhost/dashboard', 'refreshToken=true');
  ...
  describe('requireGuest', () => {
    ...
    it('should throw redirect when authenticated', () => {
      const request = createMockRequest('http://localhost/login', 'refreshToken=true');
