...
const mockInvoices: RecentInvoice[] = [
  ...
  { id: '2', number: 'INV-002', customer: 'Company B', amount: 2800000, status: 'PENDING', date: '2024-01-14' },
];

const mockAlerts: LowStockAlert[] = [
  { id: '1', name: 'Laptop HP', sku: 'HP-001', currentStock: 5, minStock: 10, warehouse: 'Main' },
  { id: '2', name: 'Mouse Logitech', sku: 'LOG-001', currentStock: 3, minStock: 8, warehouse: 'Main' },
];
...
describe('useDashboard hooks', () => {
  ...
  describe('useDashboardCharts', () => {
    it('should return loading state initially', () => {
      vi.mocked(dashboardService.getCharts).mockReturnValue(new Promise(() => {}));
  ...
  describe('useDashboard', () => {
    ...
    it('should return isError true when any query fails', async () => {
      vi.mocked(dashboardService.getStats).mockRejectedValue(new Error('Stats error'));
      vi.mocked(dashboardService.getCharts).mockResolvedValue(mockCharts);
      vi.mocked(dashboardService.getRecentInvoices).mockResolvedValue(mockInvoices);
      vi.mocked(dashboardService.getLowStockAlerts).mockResolvedValue(mockAlerts);
    ...
    it('should return undefined data for failed queries', async () => {
      vi.mocked(dashboardService.getStats).mockRejectedValue(new Error('Error'));
      vi.mocked(dashboardService.getCharts).mockResolvedValue(mockCharts);
      vi.mocked(dashboardService.getRecentInvoices).mockResolvedValue(mockInvoices);
      vi.mocked(dashboardService.getLowStockAlerts).mockResolvedValue(mockAlerts);
